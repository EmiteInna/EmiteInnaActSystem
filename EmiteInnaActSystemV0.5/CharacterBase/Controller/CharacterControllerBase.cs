...
namespace EmiteInnaACT
{
    ...
    public class CharacterControllerBase : MonoBehaviour
    {
        ...
        /// <summary>
        /// 绑定血条组件，不要在这个类里用，在子类的初始化后面用。
        /// </summary>
        /// <param name="c"></param>
        public virtual void BindGameUIController(GameUIController c)
        {
            this.gameUIController = c;
        ...
        #region Functions
        public void CharacterMove(Vector3 direction, float movementSpeed, float rotationSpeed, float accelerate = 4f)
        {
            EChara.CharacterMove(rb, direction, movementSpeed, rotationSpeed);
        }
        public void CharacterMoveWithRootMotion(Vector3 direction,float movementSpeed,float rotationSpeed,float accelerate = 4f)
        {
            EChara.CharacterMoveWithRootMotion(rb, animationController, direction, movementSpeed, rotationSpeed);
        }
        ...
        #region Battle
        /// <summary>
        /// 受到伤害时有时候可能会触发某些特殊效果，这些东西到时候用委托事件完成，这里不管，管的时候估计要很后期了。
        /// </summary>
        public virtual void TakeDamage(float healthDamage,float mentalDamage)
        {
            configure.CurrentHealth = Mathf.Max(0, configure.CurrentHealth - healthDamage);
            configure.CurrentMental = Mathf.Max(0, configure.CurrentMental - mentalDamage);
            gameUIController.UpdateAttributeBarAfterTakingDamage();
        }
        /// <summary>
        /// 同上，但是回复生命
        /// </summary>
        /// <param name="healthDamage"></param>
        /// <param name="mentalDamage"></param>
        public virtual void Regenerate(float healthRegen, float mentalRegen)
        {
            configure.CurrentHealth = Mathf.Min(configure.MaxHealth, configure.CurrentHealth +healthRegen);
            configure.CurrentMental = Mathf.Min(configure.MaxMental, configure.CurrentMental +mentalRegen);
            gameUIController.UpdateAttributeBarAfterRegenerate();
        }
        #endregion
